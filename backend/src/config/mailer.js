...
async function sendOTPEmail(email, name, otpCode) {
  try {
    ...
    const smtpUser = process.env.SMTP_USER || process.env.EMAIL_USER || 'voxscribe-sandbox@ethereal.email';

    const mailOptions = {
      from: `"VoxScribe Secure" <${smtpUser}>`,
      to: email,
      ...
      html: `
        <div style="font-family: 'Plus Jakarta Sans', 'Segoe UI', Roboto, sans-serif; max-width: 500px; margin: 0 auto; padding: 32px; background-color: #0b0f19; border: 1px solid #1e293b; border-radius: 24px; color: #f1f5f9; text-align: center;">
          <div style="display: inline-block; width: 44px; height: 44px; border-radius: 12px; background: linear-gradient(135deg, #06b6d4 0%, #4f46e5 100%); margin-bottom: 20px;">
            <!-- Simple clean icon -->
            <span style="font-size: 22px; color: white; line-height: 44px; font-weight: bold;">V</span>
          </div>
          ...
          <div style="border-t: 1px solid #1e293b; padding-top: 20px; font-size: 11px; color: #475569;">
